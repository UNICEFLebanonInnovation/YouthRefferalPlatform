...
from .form import YouthLedentPlanningForm
from .models import YouthLedent


class YouthLedentView(LoginRequiredMixin,
                          FilterView,
                          SingleTableView):
    ...
    def get_queryset(self):
        return YouthLedent.objects.filter(partner_organization=self.request.user.partner_id)
...
class AddView(LoginRequiredMixin, FormView):
    ...
    def get_success_url(self, ):
        ...
        if self.request.POST.get('save_add_another', None):
            ...
            return '/entrepreneurship/add/'
        if self.request.POST.get('save_and_continue', None):
            return '/entrepreneurship/edit/' + str(self.request.session.get('instance_id')) + '/'
        return self.success_url
...
class EditView(LoginRequiredMixin, FormView):
    ...
    #     return YouthLedentPlanningForm

    def get_form(self, form_class=None):
        # form_class = self.get_form_class()
        form = YouthLedentPlanningForm
        instance = YouthLedent.objects.get(id=self.kwargs['pk'], partner_organization=self.request.user.partner)
...
class YouthAssessment(SingleObjectMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        registry = YouthLedent.objects.get(id=self.request.GET.get('registry'),
